...
def val_one_epoch(test_loader,
                  ...
                  logger,
                  config,
                  scheduler):  # Added scheduler here
    '''
    ...
    scheduler.step(val_loss)  # For ReduceLROnPlateau or other metric-based schedulers
...
def test_one_epoch(test_loader,
    ...
    with torch.no_grad():
        for i, data in enumerate(tqdm(test_loader)):
            ...
            # Ensure correct scaling and type for saving masks
            #out = (out > config.threshold).astype(np.uint8) * 255  # Convert to binary mask
            #msk = (msk > config.threshold).astype(np.uint8) * 255  # Convert ground truth to binary mask

            # Get the index of the maximum probability class for each pixel
            out_max_indices = np.argmax(out, axis=1)  # Shape: (batch_size, height, width)
            msk_max_indices = np.argmax(msk, axis=1)  # Shape: (batch_size, height, width)

            # Convert to one-hot format
            out = np.zeros_like(out)  # Shape: (batch_size, num_classes, height, width)
            msk = np.zeros_like(msk)

           # Assign 1 to the max class, 0 to others
            np.put_along_axis(out, np.expand_dims(out_max_indices, axis=1), 1, axis=1)
            np.put_along_axis(msk, np.expand_dims(msk_max_indices, axis=1), 1, axis=1)
